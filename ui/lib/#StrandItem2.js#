function UI_EndpointItem(vhi, baseX, baseY, type) { //type can only be 3 or 5
    ...
    this.shape.on("dragend", function(pos) {
	    ...
	    this.epi.strand.invisconnection.remove();
	    ...
	    this.epi.strand.invisconnection = new Kinetic.Rect({
		    x: this.epi.strand.endptL.centerX-0.5*this.epi.strand.endptL.helixitem.sqLength,
		    y: this.epi.strand.endptL.centerY-0.5*this.epi.strand.endptL.helixitem.sqLength,
		    width: this.epi.strand.endptR.centerX-this.epi.strand.endptL.centerX+this.epi.strand.endptL.helixitem.sqLength,
		    height: this.epi.strand.endptL.helixitem.sqLength,
		    opacity: 0
		});
	    this.epi.strand.group.add(this.epi.strand.connection);
	    this.epi.strand.group.add(this.epi.strand.invisconnection);
	    this.epi.strand.invisconnection.moveToBottom();
...
function UI_StrandItem(epi1, epi2) { //must have 2 EPIs to create a strand
    ...
    this.invisconnection = new Kinetic.Rect({
	    x: this.endptL.centerX-0.5*this.endptL.helixitem.sqLength,
	    y: this.endptL.centerY-0.5*this.endptL.helixitem.sqLength,
	    width: this.endptR.centerX-this.endptL.centerX+this.endptL.helixitem.sqLength,
	    height: this.endptL.helixitem.sqLength,
	    opacity: 0
	});

function squareGrid(xGrid,yGrid) {
	...
	var x = Math.floor((e.pageX-13)/(2*r));
	var y = Math.floor((e.pageY-109)/(2*r));
	//reset color of previous circle
	if(px>=0 && px<xg && py>=0 && py<yg) {
	    grid[px][py].fill = "#F0F0F0";
	}
	//color target circle
	if(x>=0 && x<xg && y>=0 && y<yg && Math.sqrt((2*r*x+13+r-e.pageX)*(2*r*x+13+r-e.pageX)+(2*r*y+109+r-e.pageY)*(2*r*y+109+r-e.pageY))<=r) {

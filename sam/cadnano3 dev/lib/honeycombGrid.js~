function honeycombGrid(xGrid,yGrid) {
    ...
    var canvas = new Two(windowsize).appendTo(document.body);
	...
	var xLeft = Math.floor((xPos-13-r)/(rt3*r));
	var xRight = Math.ceil((xPos-13-r)/(rt3*r));
	var y = Math.floor((yPos-109)/(3*r));
	var centerxLeft = 5+r+rt3*xLeft*r;
	var centeryLeft = 5+r+3*y*r;
	if((xLeft+y)%2 === 1) {
	    centeryLeft += r;
	}
	var distLeft = Math.sqrt((xPos-centerxLeft-8)*(xPos-centerxLeft-8)+(yPos-centeryLeft-104)*(yPos-centeryLeft-104));
	var centerxRight = 5+r+rt3*xRight*r;
	var centeryRight = 5+r+3*y*r;
	if((xRight+y)%2 === 1) {
	    centeryRight += r;
	}
	var distRight = Math.sqrt((xPos-centerxRight-8)*(xPos-centerxRight-8)+(yPos-centeryRight-104)*(yPos-centeryRight-104));
	if(distLeft <= r) {
	    result[0] = xLeft;
	    result[1] = y;
	}
    ...
    var px = 0;
    var py = 0;
    document.body.onmousemove = handlerMM;
    ...
    document.body.onclick = handlerMC;
